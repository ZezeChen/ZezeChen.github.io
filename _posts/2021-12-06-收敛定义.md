...
这个例子说明了依测度收敛定义中的$\epsilon$换成$0$会导致含义上的改变。注意到$[|f_n(x)-f(x)| > \epsilon]\subset [|f_n(x)-f(x)| > 0]$，由测度的单调性，有$m([|f_n(x)-f(x)| > \epsilon]) \leq m([|f_n(x)-f(x)| > 0])$，由夹逼定理，可以得到：若$m([|f_n(x)-f(x)| > 0])\to 0$，则$\begin{aligned}f_n \stackrel{m}{\longrightarrow} f\end{aligned}$.
