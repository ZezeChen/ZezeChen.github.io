...
> **Dirichlet判别法** 设 $$F(A)=\int_a^A f(x)dx$$
>
>  在 $$[a,\infty)$$ 上有界，函数 $$g(x)$$ 在 $$[a,\infty)$$ 上单调，且 $$\lim_{x \to + \infty} g(x) =0$$ ，则积分 $$\int_a^{+ \infty} f(x)g(x)dx$$ 收敛。
...
$$
又因为 $$f$$ 积分收敛，故 $$\forall \epsilon >0$$ ， $$\exists M >0$$ 使得 $$A,B>M$$ 时
$$
